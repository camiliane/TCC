...
#sapply("Tubular adenocarcinoma", compare_deg, case2="Mucinous adenocarcinoma")
```

```{r - summary of up- down- counts}
files <- list.files("RESULTADOS STAD/Summary_ups_downs_notsig/", recursive=TRUE, full.names=TRUE)
name <- list("IA x AC-NOS", "IA x MA", "IA x SRCC",
              "IA x TA", "AC-NOS x MA", "AC-NOS x SRCC", 
              "AC-NOS x TA", "DC x IA", "DC x AC-NOS",
            "DC x MA", "DC x SRCC", "DC x TA", 
            "SRCC x MA", "SRCC x TA", "TA x MA")
...
summary_all <- do.call(rbind, myfilelist)
row.names(summary_all) <- name

#write.table(summary_all, file = "Summary_ups_downs_notsigs/summary_all.txt", quote = F, sep = "\t", row.names = T, eol = "\r")
```

```{r - deg for cftr and mucins}
files <- list.files("DEG_CFTR_MUCs/", recursive=TRUE, full.names=TRUE)

myfilelist <- lapply(files, reader)

df <- do.call(rbind, myfilelist)
cftr <- df[df$symbol=="CFTR",]
rownames(cftr) <- names

#write.table(cftr, file = "DEG_CFTR_MUCs/summary_cftr.txt", quote = F, sep = "\t", row.names = T, eol = "\r")
```
